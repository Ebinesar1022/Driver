...
(function () {
  ...
  function go(view) {
    ...
    ["Dash", "Vcheck", "Chktyres", "Chkbattery", "Chkfuel", "Chkgps", "Chkhealth",
     "Trip", "CheckIn", "Pod", "Fuel", "Incident", "VehicleIssue", "Break", "TripFeedback",
  ...
  var VCHECKS = {
    ...
    fuel: { label: "Fuel", fields: ["fVcFuelType", "fFuelPct", "fFuelOk", "fVcOdo"] },
  ...
  function saveVcheck() {
    ...
    ZOHO.CREATOR.DATA.addRecords({
      ...
      payload: {
        data: {
          Trip_ID: creatorLookupId(ACTIVE_TRIP.tripRecordId || ACTIVE_TRIP.tripId),
          Driver: creatorLookupId(ACTIVE_TRIP.driverRecordId || ACTIVE_TRIP.driverEmployeeRecordId || ACTIVE_TRIP.driverName || ACTIVE_TRIP.driverId || ""),
          Trip_Name: creatorLookupId(ACTIVE_TRIP.tripRecordId || ACTIVE_TRIP.tripId),
          Vehicle: creatorLookupId(ACTIVE_TRIP.vehicleRecordId || ACTIVE_TRIP.vehicleName),
  ...
  /* ------------------------------ trip start ------------------------------ */
  // Starts the trip directly the moment the driver taps the "Start Trip"
  // CTA on the vehicle-check page (no separate Start Trip page/form any
  // more). Start time is the current clock time, live location/URL are
  // captured automatically off the device GPS (best-effort — a denied or
  // slow GPS fix never blocks the driver from starting), and the starting
  // odometer reading comes from the field now on the pre-trip Fuel check.
  function startTripNow() {
    // Guard: the CTA is disabled until all 5 checks are verified, but the
    // check is repeated here in case this is ever invoked another way.
    if (!Object.keys(vcVerified).every(function (k) { return vcVerified[k]; })) {
      toast("Complete all 5 vehicle checks before starting the trip.");
      return;
    }
    ...
    if (!tripRecordId || !driverRecordId) {
      toast("Can't start — no assigned trip/driver is loaded yet. Please wait for the page to finish loading and try again.");
      return;
    }

    // Starting odometer now lives on the Fuel check card (id inVcOdo) —
    // its presence is already enforced by gradeVcheck()/verifyCheck()
    // before the fuel check (and therefore all 5 checks) can pass.
    var startOdometerNum = Number(val("#inVcOdo")) || 0;
    ...
    // End Location is fetched automatically from the assigned
    // Trip_Dispatch1 record's To_Location field — never typed by the driver.
    var endLocation = ACTIVE_TRIP.record ? (tripField(ACTIVE_TRIP.record, "toLocation") || "") : "";

    function commitTripStart(startLocation, startLocationUrl) {
      ...
      // the assigned trip / the driver's captured GPS location — nothing
      // hard-coded, nothing regenerated.
      //
      // NOTE — field-name fix: this used to write "Live_location" /
      // "Live_location_URL" (lowercase "location"), which does not match
      // this app's actual Live Location field naming convention
      // (Live_Location / Live_Location_URL, capital L — confirmed against
      // every other form in this app: Fuel_Entry, Incident_Report,
      // Vehicle_Issue, Break). That casing mismatch is why the live
      // location/URL silently failed to save. Only sent when a real GPS
      // fix was captured — never a placeholder string.
      var payload = {
        Trip_ID: creatorLookupId(tripRecordId || ACTIVE_TRIP.tripId),
        Trip_Name: creatorLookupId(tripRecordId || ACTIVE_TRIP.tripId),
        Driver_ID: creatorLookupId(driverRecordId || ACTIVE_TRIP.driverEmployeeRecordId || ACTIVE_TRIP.driverId || ACTIVE_TRIP.driverName || ""),
        Driver_Name: creatorLookupId(driverRecordId || ACTIVE_TRIP.driverEmployeeRecordId || ACTIVE_TRIP.driverName || ACTIVE_TRIP.driverId || ""),
        Start_Date_Time: formatDateTimeForCreator(startDateTime),
        End_location: endLocation,
        Starting_Odometer_Reading: startOdometerNum,
      };
      if (startLocation) payload.Live_Location = startLocation;
      if (startLocationUrl) payload.Live_Location_URL = startLocationUrl;
    ...
    // Auto-capture live location from the device GPS. Best-effort and
    // time-boxed: a denied permission, unavailable GPS, or slow fix must
    // never block the driver from starting the trip — it just starts
    // without a location on this record rather than stalling.
    if (navigator.geolocation) {
      toast("Starting trip — capturing your location…");
      var settled = false;
      ...
      navigator.geolocation.getCurrentPosition(function (pos) {
        ...
        commitTripStart(la + ", " + lo, "https://maps.google.com/?q=" + la + "," + lo);
      }, function () {
        if (settled) return;
        settled = true;
        clearTimeout(timeoutId);
  ...
  function prefillFuel() {
    ...
    // Trip ID is always the current started trip — read-only, never typed
    // or changed by the driver.
    var tripIdEl = $("#inFuelTripId");
    if (tripIdEl) tripIdEl.value = ACTIVE_TRIP.tripId || CURRENT_TRIP_ID || "";
    recalcFuelTotal();
  ...
  function saveFuel() {
    console.log(LOG_TAG, "saveFuel() called");
    var err = $("#fuelErr");
    err.hidden = true;
    ["fFuelCurrentLoc", "fFuelType", "fFuelQty", "fFuelCost", "fFuelOdo", "fFuelUrl"].forEach(function (id) {
      var el = document.getElementById(id);
    ...
    // names (from the actual Fuel_Entry form definition):
    //   Trip_ID (picklist -> Trip_Dispatch.ID), Driver_ID (picklist ->
    //   Driver_Details.ID), Driver_Name (picklist ->
    //   Employee_Form[Designation=="Driver"].ID), Vehicle (picklist ->
    //   Vehicles_Report1.ID), Fuel_Entry_Date_Time, Current_location,
    //   Fuel_Type, Cost_Per_Unit, Total_Fuel_Cost, Fuel_Station,
    //   Live_Location_URL, Odometer_Reading, Fuel_Quantity_L_kWh.
    //
    // FIELD-NAME FIX: this previously wrote "Trip" and "Driver", which are
    // not real fields on this form — the actual API names are "Trip_ID"
    // and "Driver_Name". Since neither field is "must have" on the form,
    // Creator silently accepted the save while dropping those two values,
    // which is why Trip ID never actually landed on the record even
    // though the rest of the entry appeared to save fine.
    //
    // Every lookup value comes from ACTIVE_TRIP, locked in once when the
    // driver started the trip and never regenerated — so Trip_ID,
    // Vehicle, Driver_ID and Driver_Name stay identical across every
    // fuel entry for the whole trip.
    var payloadData = {
      ...
      Vehicle: creatorLookupId(ACTIVE_TRIP.vehicleRecordId || ACTIVE_TRIP.vehicleName),
      Trip_ID: creatorLookupId(ACTIVE_TRIP.tripRecordId || ACTIVE_TRIP.tripId),
      // Driver_ID: creatorLookupId(ACTIVE_TRIP.driverId || ACTIVE_TRIP.driverRecordId || ""),
      Driver_Name: creatorLookupId(ACTIVE_TRIP.driverRecordId || ACTIVE_TRIP.driverEmployeeRecordId || ACTIVE_TRIP.driverName || ACTIVE_TRIP.driverId || ""),
    ...
    console.log(LOG_TAG, "Fuel_Entry payload before normalizing live location URL:", payloadData);
    
    var liveUrl = normalizeLiveLocationUrl(entry.liveLocationUrl);
    console.log(LOG_TAG, "Normalized live location URL:", liveUrl);
    if (liveUrl) payloadData.Live_Location_URL = liveUrl;
    console.log(LOG_TAG, "Fuel_Entry payload:", payloadData);
  ...
  // Normalizes a location URL the driver typed or captured: trims it and,
  // if it's non-empty but missing a URL scheme, prefixes "https://" rather
  // than dropping the value outright — a bare "maps.google.com/?q=…" or
  // pasted share link without "https://" is still a usable, storable URL.
  function normalizeLiveLocationUrl(raw) {
    var u = (raw || "").trim();
    if (!u) return "";
  ...
  function pushToCreator(formName, data) {
    ...
    if (data.Trip_ID === undefined) data.Trip_ID = creatorLookupId(ACTIVE_TRIP.tripRecordId || ACTIVE_TRIP.tripId || CURRENT_TRIP_ID || "");
    if (data.Vehicle === undefined) data.Vehicle = creatorLookupId(ACTIVE_TRIP.vehicleRecordId || ACTIVE_TRIP.vehicleName || "");
    if (data.Driver === undefined) data.Driver = creatorLookupId(ACTIVE_TRIP.driverRecordId || ACTIVE_TRIP.driverEmployeeRecordId || ACTIVE_TRIP.driverName || ACTIVE_TRIP.driverId || driver.name || driver.id || "");
    if (data.Driver_ID === undefined) data.Driver_ID = creatorLookupId(ACTIVE_TRIP.driverRecordId || ACTIVE_TRIP.driverEmployeeRecordId || ACTIVE_TRIP.driverId || driver.id || "");
  ...
  function luId(v) {
    ...
    return String(v.ID || v.zc_id || v.id || "").trim();
  }
  function creatorLookupId(v) {
  ...
  function boot() {
    ...
    on("#btnStartTripCta", "click", startTripNow);
